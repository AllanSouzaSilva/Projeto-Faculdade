...
{
    public partial class TelaGerarCadastros : Form
    {
        ...
        private void DisplayData()
        {
            ...
            adapt = new SqlDataAdapter("SELECT * FROM TB_Cliente ORDER BY Id_Cliente DESC", con);
            adapt.Fill(dt);
            dataGridRelatorioCadastros.DataSource = dt;
            con.Close();
        }
        ...
        {
            this.Close();
            GerarTransacoess = new Thread(OpenTransacoes);
            GerarTransacoess.SetApartmentState(ApartmentState.STA);
            GerarTransacoess.Start();
        }
